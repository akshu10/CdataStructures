...
// This method is the recursive method for searching a value on the tree.
...
void bintree_print_breadth_first(BinaryTree *binTree) {
    ...
    Queue *queue = queue_initialize(sizeof(*binTree->top), "void");
...
void bintree_print_depth_first(BinaryTree *binTree) {
    ...
    Stack *stack = stack_initialize((sizeof(*binTree->top)), "void");
...
bool _bintree_insert_replace_recursive(BinaryTree *binTree, BinaryTreeNode *node, void *pointer) {
    ...
    if (((c > 0)) && (node->right != NULL)) {
        ...
        if (c == 0) {
            ...
            node->left = bintree_create_node((binTree->itemSize), pointer);
        } else {

            node->right = bintree_create_node((binTree->itemSize), pointer);
        }
